...
# Data augmentation by masking
def augment_data(X, y, mask_prob=0.1):
    X_augmented = X.copy()
    mask = np.random.rand(*X.shape) < mask_prob
    X_augmented[mask] = 0  # Masking by setting selected values to zero
    return np.concatenate([X, X_augmented]), np.concatenate([y, y])

# Apply the masking data augmentation
X_train_aug, y_train_aug = augment_data(X_train, y_train, mask_prob=0.01)

print(f"X_train_aug shape: {X_train_aug.shape}")
print(f"y_train_aug shape: {y_train_aug.shape}")

# Function to find masked sequences
def find_masked_sequences(X_augmented):
    masked_sequences_indices = []
    for i, sequence in enumerate(X_augmented):
        if np.any(sequence == 0):  # Check if there is any masked value (i.e., value is zero)
            masked_sequences_indices.append(i)
    return masked_sequences_indices

# Find and print indices of masked sequences
masked_sequences_indices = find_masked_sequences(X_train_aug)

print(f"Total masked sequences found: {len(masked_sequences_indices)}")
print("Indices of masked sequences:")
print(masked_sequences_indices)

# Print the first few masked sequences for inspection
for idx in masked_sequences_indices[:5]:  # Display the first 5 masked sequences
    print(f"Masked sequence at index {idx}:\n{X_train_aug[idx]}")
...
tuner = kt.BayesianOptimization(
    ...
    project_name='rul_prediction1Transformer-CNN-maskAugment',
